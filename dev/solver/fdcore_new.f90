...
real,    private, allocatable :: u(:,:), w(:,:), p(:,:), csg(:,:), &
                                 p_end(:,:), u_end(:,:), w_end(:,:), boundary(:,:),alpha1(:,:),alpha2(:,:)
...
subroutine staggered42_modeling_is(is, par, coord, s, c, den, fs, nx_pml, nz_pml, npml, damp, store_boundary)
  ...
  allocate(alpha1(nz_pml,nx_pml))
  allocate(alpha2(nz_pml,nx_pml))
  ...
  do ix=1,nx_pml
    do iz=1,nz_pml
      alpha1(iz,ix) = den(iz,ix)*c(iz,ix)*c(iz,ix)*par%dt/(par%dx)
      alpha2(iz,ix) = par%dt/(den(iz,ix)*par%dx)
    enddo
  enddo
  ! Time marching loop
  do it=2,par%nt
    ...
    do ix=3,nx_pml-2
      do iz=fs(ix)+1,nz_pml-2
        ...
        ! p(iz,ix) = (1.0-kappa)*p(iz,ix) - &
        ! (c1_staggered*(alpha1(iz,ix)*u(iz,ix)-alpha1(iz,ix-1)*u(iz,ix-1) +&
        !                                 alpha1(iz,ix)*w(iz,ix)-alpha1(iz-1,ix)*w(iz-1,ix)) + &
        ! c2_staggered*(alpha1(iz,ix+1)*u(iz,ix+1)-alpha1(iz,ix-2)*u(iz,ix-2) + &
        !                                 alpha1(iz+1,ix)*w(iz+1,ix)-alpha1(iz-2,ix)*w(iz-2,ix)))
      enddo
    enddo
    ...
    do ix=2,nx_pml-2
      do iz=fs(ix),nz_pml-1
        ...
        u(iz,ix) = (1.0-kappa)*u(iz,ix) -alpha*(c1_staggered*(p(iz,ix+1)-p(iz,ix)) &
          ...
          ! u(iz,ix) = (1.0-kappa)*u(iz,ix) -(c1_staggered*(alpha2(iz,ix+1)*p(iz,ix+1)-alpha2(iz,ix)*p(iz,ix)) &
          !           +c2_staggered*(alpha2(iz,ix+2)*p(iz,ix+2)-alpha2(iz,ix-1)*p(iz,ix-1)))
      enddo
    enddo
    ...
    do ix=2,nx_pml-1
      do iz=fs(ix),nz_pml-2
        ...
        ! w(iz,ix) = (1.0-kappa)*w(iz,ix) - (c1_staggered*(alpha2(iz+1,ix)*p(iz+1,ix)-alpha2(iz,ix)*p(iz,ix)) &
        !                   +c2_staggered*(alpha2(iz+2,ix)*p(iz+2,ix)-alpha2(iz-1,ix)*p(iz-1,ix)))
      enddo
    enddo
    ...
    !if (is == 15) then
    !  call filename(output, '/home/guangyuan/桌面/ttt/s', it, '.bin')
    !  call write_binfile(output,  p(npml+1:nz_pml-npml,npml+1:nx_pml-npml),par%nz, par%nx)
    !endif
...
subroutine staggered42_back_it(is, it, par, coord, s, c, den, fs, nx_pml, nz_pml, npml, damp, p, u, w)
  ...
  do ix=2,nx_pml-2
    do iz=2,nz_pml-1
      ...
      !                                           +c2_staggered*(p(iz,ix+2)-p(iz,ix-1)))
      !  Diff coef
      u(iz,ix) = (1.0-kappa)*u(iz,ix) - (c1_staggered*(alpha1(iz,ix+1)*p(iz,ix+1)-alpha1(iz,ix)*p(iz,ix)) &
                                                  +c2_staggered*(alpha1(iz,ix+2)*p(iz,ix+2)-alpha1(iz,ix-1)*p(iz,ix-1)))
    enddo
  enddo
  ! Update verticle particle velocity: w
  do ix=2,nx_pml-1
    do iz=2,nz_pml-2
      ...
      kappa = 0.5*(damp(iz,ix)+damp(iz+1,ix))*par%dt
       ...
       !                                          +c2_staggered*(p(iz+2,ix)-p(iz-1,ix)))
       !  Diff coef 
      w(iz,ix) = (1.0-kappa)*w(iz,ix) - (c1_staggered*(alpha1(iz+1,ix)*p(iz+1,ix)-alpha1(iz,ix)*p(iz,ix)) &
                                                    +c2_staggered*(alpha1(iz+2,ix)*p(iz+2,ix)-alpha1(iz-1,ix)*p(iz-1,ix)))
    enddo
  enddo
  ...
  do ix=3,nx_pml-2
    do iz=3,nz_pml-2
      ...
      !  Diff coef 
      p(iz,ix) = (1.0-kappa)*p(iz,ix) -  &
          (c1_staggered*(alpha2(iz,ix)*u(iz,ix)-alpha2(iz,ix-1)*u(iz,ix-1) + &
          alpha2(iz,ix)*w(iz,ix)-alpha2(iz-1,ix)*w(iz-1,ix))+&
            c2_staggered*(alpha2(iz,ix+1)*u(iz,ix+1)-alpha2(iz,ix-2)*u(iz,ix-2) +&
             alpha2(iz+1,ix)*w(iz+1,ix)-alpha2(iz-2,ix)*w(iz-2,ix)))
    enddo
  enddo
