...
misfit_data   = np.loadtxt("./LBFGS/outputs/misfit_data.dat")
vp_true       = np.loadtxt('./model/Marmousi_481_141_25m.dat').T

# load FWI models
for it in range(maxiter):
    vp_FWI = loadbinfloat32('./LBFGS/outputs/velocity/vp-%d.bin'%(it+1)).reshape((nx, nz)).T
...
plt.savefig('misfit-LBFGS.png', dpi=300)
