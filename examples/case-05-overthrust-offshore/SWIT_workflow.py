...
### system setup
modelpath = '/data/haipeng/SWIT-1.0/examples/case-05-overthrust-offshore/model/'   # Model path, i.e., velocity, source & receiver coordinates
homepath  = '/data/haipeng/SWIT-1.0/examples/case-05-overthrust-offshore/'         # Working path
mpiproc   = 51                                     # MPI process for fd2dmpi
...
dx,  dt,  nt = [20, 0.0015, 3001]                  # Grid size, time interval, and time step
...
# true model
vp_true = np.loadtxt(modelpath + 'Overthrust_501_171_20m.dat')

# initial model
vp_init = np.copy(vp_true)
vp_init[:,20:] = smooth2d(vp_true[:,20:], span = 20)
np.savetxt(modelpath + 'Overthrust_501_171_20m_smooth20.dat', vp_init)
...
vpmax       = 6100                                 # maximum allowed velocity
...
grad_smooth = 4                                    # gradient smooth radius 
