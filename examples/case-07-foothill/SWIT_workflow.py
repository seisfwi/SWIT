...
### system setup
modelpath = '/data/haipeng/SWIT-1.0/examples/case-07-foothill/model/'   # Model path, i.e., velocity, source & receiver coordinates
homepath  = '/data/haipeng/SWIT-1.0/examples/case-07-foothill/'         # Working path
mpiproc   = 60                                     # MPI process for fd2dmpi
figaspect = 1.0                                    # Figure aspect

### model setup
nx,  nz      = [801,  291]                         # Grid number along x and z directions
...
# true model
vp_true = np.loadtxt(modelpath + 'Foothill_801_331_25m.dat')[:nx,:nz]

# initial model
vp_init = np.copy(vp_true)
vp_init = smooth2d(vp_true, span = 25)
vp_init[vp_true[:,:]==1500]=1500
np.savetxt(modelpath + 'Foothill_801_331_25m_smooth25.dat', vp_init)
...
### sources setup 
f0    = 3.0                                              # Dominant frequency in Hz
...
vpmax       = 6000                                 # maximum allowed velocity
