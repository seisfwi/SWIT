...
### system setup
homepath  = '/data/haipeng/SWIT-1.0/examples/case-09-RTM/'   # working path
mpiproc   = 90                                     # mpi process for fd2dmpi
figaspect = 1.0                                    # Figure aspect

### model setup
nx,  nz      = [1201, 515]                         # Grid number along x and z directions
pml, fs      = [50,  True]                         # Grid number for PML layers (use a large one) and free surface
dx,  dt,  nt = [10, 0.001, 6001]                   # Grid size, time interval, and time step
...
# true model
vp_true = np.loadtxt(homepath + 'model/SEAM_I_1201_515_10m.dat')

# initial model
vp_init = np.copy(vp_true)
vp_init = smooth2d(vp_true, span = 15)
vp_init[vp_true[:,:]==1490]=1490
np.savetxt(homepath + 'model/SEAM_I_1201_515_10m_smooth15.dat', vp_init)
...
### sources setup 
f0    = 15.0                                        # Dominant frequency in Hz
...
### inversion parameter
misfit_type = 'RTM'                                # 'Traveltime', 'Waveform', 'Globalcorrelation'
scheme      = 'NLCG'                               # 'NLCG', 'LBFGS'
maxiter     = 100                                  # maximum iteration number,  i.e., 20
...
grad_mute = 0                                      # mute source energy for 'Land', or water mask for 'Marine'
...
# mute later arrival
mute_late_arrival = True                           # pick first break and mute later arrival
...
optim = base.optimize(misfit_type, scheme, maxiter, step_length, vpmax, vpmin, marine_or_land,
                ...
                mute_offset_short_dis, mute_offset_long_dis, grad_mask=grad_mask)
