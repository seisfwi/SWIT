...
### system setup
homepath  = '/data/haipeng/SWIT-1.0/examples/marine-case2-overthrust/'   # working path
mpiproc   = 41                                     # mpi process for fd2dmpi
figaspect = 1.0                                    # Figure aspect

### model setup
nx,  nz      = [401,  121]                         # Grid number along x and z directions
...
# true model
vp_true = np.loadtxt(homepath + 'model/Overthrust_401_121_25m_True.dat')

# initial model
vp_init = np.copy(vp_true)
vp_init[:,20:] = smooth2d(vp_true[:,20:], span = 20)
np.savetxt(homepath + 'model/Overthrust_401_121_25m_True_smooth_20.dat', vp_init)
